...
import { Fragment, useMemo, type ComponentPropsWithoutRef, type FC } from "react";
...
export const ThreadListItems: FC<ComponentPropsWithoutRef<"div">> = ({
  ...
  return (
    <div
      ...
      className={cn("flex flex-col gap-0.5 group-data-[collapsible=icon]:hidden", className)}
      {...props}
    >
...
function dateGroupLabel(date: Date | undefined, startOfToday: number): string {
...
const ThreadListItemGroups: FC = () => {
  ...
  const groups = useMemo<ThreadListGroup[] | null>(() => {
    const itemsById = new Map(
      Object.values(threadItems).map((item) => [item.id, item]),
    );
        ...
        {children ?? (
          <>
            ...
            <span
              ...
              className={cn("whitespace-nowrap group-data-[collapsible=icon]:hidden", labelClassName)}
            >
              新建任务
            </span>
          </>
