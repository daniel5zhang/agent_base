...
11. 用户输入、prompt、模型输出、工具调用、插件调用、业务查询、数据导出、权限判断、审批和配置变更必须进入审计；审计查看本身也必须受权限控制。
...
审计留存周期可配置：默认 180 天，高风险/业务数据访问默认 1 年，监管或公司制度要求可配置更长。
防篡改能力需要预留：每条审计事件记录 hash，按批次生成 hash chain；Phase 1 先写 SQLite，生产阶段接入对象存储/WORM/日志审计平台。
审计查看需要权限控制，普通用户不能查看超出自身权限的数据。
审计检索第一批支持时间范围、用户、插件、工具、业务系统、审批单号、审计编号、风险等级、操作类型。
审计导出第一批支持 CSV 和 JSON，但导出本身需要权限判断和审计。
审计查看权限：普通用户只能查看自己的操作审计摘要；团队管理员查看团队范围审计；租户管理员查看租户范围审计；审计管理员查看被授权范围内的完整审计；系统管理员管理审计配置，不默认查看业务敏感明细。
敏感字段默认脱敏展示，具备权限时可以查看明文；查看明文本身需要审计；无权限时只能看到脱敏值或 hash。
```
...
```text
全部插件：展示当前用户可见的所有插件。
已安装：展示当前用户/团队/租户已安装插件。
待授权：展示需要 OAuth、SSO、API Key、数据权限或审批的插件。
可升级：展示有新版本的插件。
已停用：展示用户或管理员停用的插件。
管理员管理：仅团队管理员、租户管理员、系统管理员可见，用于安装租户级插件、分配可见范围、启停插件、发布策略。
审计记录：展示插件安装、授权、启停、升级、调用记录；普通用户只能看自己的，管理员按权限看团队/租户。
```
