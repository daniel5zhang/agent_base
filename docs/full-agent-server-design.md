...
4. 设置页是否要暴露多 Model Provider 配置；已确认：采用 UI 可配置方案。设置页需要支持新增、编辑、禁用、删除多个 Model Provider，字段包括 provider 名称、provider 类型、OpenAI-compatible base URL、API Key、模型列表、默认模型、启用状态和适用范围。API Key 只能提交给服务端保存，前端不得持久化明文密钥；服务端需要提供密钥加密、脱敏展示、连通性测试、租户级/用户级授权和审计记录。Phase 1B 先实现后端 `ModelProvider` 抽象和配置 API，前端设置页改动需单独列出方案并再次确认后开发。
5. 插件 catalog 是否要在前端增加安装、授权、启停管理界面；已确认：采用完整插件管理界面方案。前端需要建设“插件中心 / 插件管理”能力，支持插件列表、插件详情、安装、授权、启用/停用、版本信息、风险等级、能力列表、权限申请和审计记录。服务端需要提供插件 catalog、installation、authorization、enable/disable、release policy、tenant/user visibility、audit API。插件中心需要区分普通用户和管理员权限。该能力涉及明确前端 UI 改动和服务端插件平台 API，开发前需要单独输出界面结构、接口 schema、权限边界和阶段实施方案并再次确认。
