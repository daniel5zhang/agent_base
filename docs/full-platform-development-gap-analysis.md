...
当前文档已经可以支撑 Phase 1B 的 Agent Runtime 开发。经过逐项确认后，完整平台的关键产品决策也已补齐，可以进入“完整平台任务级开发计划”编写阶段：
...
剩余工作不是继续做产品方向确认，而是把已确认内容细化成可执行工程计划：
...
本文档已完成“缺口清单”和“需确认项”的产品决策补齐。下一步应输出完整平台任务级开发计划。
...
| Model Provider | 已确认方案 B、入口、角色、模型列表策略 | 需要接口 schema、密钥加密实现、设置页组件方案 | 否 |
| 插件中心 | 已确认入口、分区、角色、状态机 | 需要接口 schema、页面组件方案、安装授权流程细化 | 否 |
| 审批中心 | 已确认审批对象、mock 闭环、过期、provider 优先级 | 需要审批状态机、接口 schema、右侧 Tab 组件方案 | 否 |
| 右侧业务面板 | 已确认 Artifact 卡片、Tab、renderer、下载、错误态 | 需要 renderer 注册接口和默认组件方案 | 否 |
| 企业账号权限 | 已确认自有账号体系、RBAC+ABAC、角色、数据权限粒度 | 需要数据库模型和策略计算接口 | 否 |
| 审计与监管 | 已确认记录范围、留存、检索、导出、脱敏、防篡改 | 需要审计表结构、检索接口和导出任务设计 | 否 |
| Connector Runtime | 已确认 Codex-style 主参考、三档接入、执行位置、授权、返回结构、动态工具池 | 需要 Connector SDK/API schema、错误码、适配器实现计划 | 否 |
| 前端 Runtime 迁移 | 保留待定 | 服务端完成后再评估 AssistantTransport / AG-UI | 后置决策 |
...
- 全部插件；已确认：展示当前用户可见的所有插件。
- 已安装；已确认：展示当前用户/团队/租户已安装插件。
- 待授权；已确认：展示需要 OAuth、SSO、API Key、数据权限或审批的插件。
- 可升级；已确认：展示有新版本的插件。
- 已停用；已确认：展示用户或管理员停用的插件。
- 管理员管理；已确认：仅团队管理员、租户管理员、系统管理员可见，用于安装租户级插件、分配可见范围、启停插件、发布策略。
- 审计记录；已确认：展示插件安装、授权、启停、升级、调用记录；普通用户只能看自己的，管理员按权限看团队/租户。
...
- 审批拒绝后是否允许修改范围后重新提交；已确认：允许修改范围后重新提交。
- 审批过期时间；已确认：默认 7 天，可配置。
- 审批记录是否在聊天中可见；已确认：主对话展示审批摘要卡片。
- 审批记录是否进入右侧业务面板长期保留；已确认：右侧审批 Tab 长期保留完整审批详情，主对话卡片可重新打开。
...
- 钉钉；已确认：第二优先级。
- 企业微信；已确认：第三优先级。
- 仅预留 provider；已确认：保留 provider 抽象，便于后续扩展。
...
- Tab 关闭策略；已确认：Tab 可以关闭，但关闭不删除 Artifact；主对话结果卡片可重新打开对应 Artifact；如果 Artifact 有未保存表单内容，关闭前提示。
...
- Artifact 是否可分享；已确认：默认不支持外部分享，仅支持当前会话内通过主对话卡片重新打开；后续如果需要分享，必须经过权限判断和审计。
- renderer 注册机制；已确认：由插件 manifest 声明 renderer，服务端返回 `renderer_hint`，前端根据 `renderer_hint` 找到对应业务组件；找不到 renderer 时展示通用 JSON / 表格 fallback。
- 空面板状态；已确认：提示“从对话中的结果卡片打开业务内容”。
- 错误状态；已确认：展示错误原因、重试按钮、审计/事件编号。
- 审计编号展示位置；已确认：固定展示在 Artifact 详情顶部元信息区和底部审计区。
- 业务表格、图表、表单、审批的默认组件规范；已确认：第一批默认组件包括 Table、Metric、Chart、Form、Approval、QueryPlan、AuditInfo、ErrorState。
...
权限缓存：
  - 权限变更后立即失效缓存
  - 登录态可继续，但下一次权限判断必须读取最新权限
```

是否需要用户确认：已确认。尤其是角色体系和组织权限粒度。
...
- 是否支持导出；已确认：支持，但导出本身需要权限判断和审计。第一批导出格式支持 CSV 和 JSON。
- 是否支持检索；已确认：支持。第一批检索条件包括时间范围、用户、插件、工具、业务系统、审批单号、审计编号、风险等级、操作类型。
...
- 敏感字段如何脱敏；已确认：默认脱敏展示，具备权限时可以查看明文；查看明文本身需要审计；无权限时只能看到脱敏值或 hash。
- 管理员查看审计的权限边界；已确认：普通用户只能查看自己的操作审计摘要；团队管理员查看团队范围审计；租户管理员查看租户范围审计；审计管理员查看被授权范围内的完整审计；系统管理员管理审计配置，不默认查看业务敏感明细。
