...
- 设置页是放在左下角“设置与模型”中，还是作为右侧业务面板 Tab 打开；
- 是否区分“个人模型配置”和“企业模型配置”；
- 普通用户是否可以新增 Provider；
- 管理员配置的 Provider 是否可以被普通用户覆盖；
- 模型列表是手动填写，还是支持从 Provider 拉取。
...
- Phase 1C mock 审批是“本地模拟通过/拒绝”，还是只创建申请记录；
- 审批通过后是否自动恢复原 run；
- 审批拒绝后是否允许修改范围后重新提交；
- 审批过期时间；
- 审批记录是否在聊天中可见；
